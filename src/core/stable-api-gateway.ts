...
export async function stableApiGateway<RequestDataType = any, ResponseDataType = any>(
    ...
    if (concurrentExecution) {
        return executeConcurrently<RequestDataType, ResponseDataType>(requests, requestExecutionOptions as CONCURRENT_REQUEST_EXECUTION_OPTIONS<RequestDataType, ResponseDataType>);
    } else {
