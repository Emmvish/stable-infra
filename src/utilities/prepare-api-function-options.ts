...
export function prepareApiFunctionOptions<TArgs extends any[], TReturn>(
    ...
    const functionOptions: Omit<STABLE_FUNCTION<TArgs, TReturn>, 'fn' | 'args'> = {
        ...
        preExecution: func.functionOptions.preExecution ?? (funcGroup?.commonConfig?.commonFunctionPreExecution as any) ?? (gatewayOptions.commonFunctionPreExecution as any),
    ...
    if (!func.functionOptions.handleErrors && funcGroup?.commonConfig?.commonHandleFunctionErrors) {
        functionOptions.handleErrors = funcGroup.commonConfig.commonHandleFunctionErrors as any;
    } else if (!func.functionOptions.handleErrors && gatewayOptions.commonHandleFunctionErrors) {
        functionOptions.handleErrors = gatewayOptions.commonHandleFunctionErrors as any;
    }
    
    if (!func.functionOptions.handleSuccessfulAttemptData && funcGroup?.commonConfig?.commonHandleSuccessfulFunctionAttemptData) {
        functionOptions.handleSuccessfulAttemptData = funcGroup.commonConfig.commonHandleSuccessfulFunctionAttemptData as any;
    } else if (!func.functionOptions.handleSuccessfulAttemptData && gatewayOptions.commonHandleSuccessfulFunctionAttemptData) {
        functionOptions.handleSuccessfulAttemptData = gatewayOptions.commonHandleSuccessfulFunctionAttemptData as any;
    }
    
    if (!func.functionOptions.responseAnalyzer && funcGroup?.commonConfig?.commonFunctionResponseAnalyzer) {
        functionOptions.responseAnalyzer = funcGroup.commonConfig.commonFunctionResponseAnalyzer as any;
    } else if (!func.functionOptions.responseAnalyzer && gatewayOptions.commonFunctionResponseAnalyzer) {
        functionOptions.responseAnalyzer = gatewayOptions.commonFunctionResponseAnalyzer as any;
    }
    
    if (!func.functionOptions.finalErrorAnalyzer && funcGroup?.commonConfig?.commonFinalFunctionErrorAnalyzer) {
        functionOptions.finalErrorAnalyzer = funcGroup.commonConfig.commonFinalFunctionErrorAnalyzer as any;
    } else if (!func.functionOptions.finalErrorAnalyzer && gatewayOptions.commonFinalFunctionErrorAnalyzer) {
        functionOptions.finalErrorAnalyzer = gatewayOptions.commonFinalFunctionErrorAnalyzer as any;
    }
