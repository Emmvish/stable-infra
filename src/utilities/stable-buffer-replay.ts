...
import { ReplaySkipReasons } from '../enums/index.js';
import type {
  BufferLike,
...
export async function replayStableBufferTransactions(
  ...
  for (const log of orderedLogs) {
    if (activityFilter && !activityFilter(log)) {
      skipped += 1;
      onSkip?.(log, ReplaySkipReasons.FILTERED);
      continue;
    }

    if (dedupe && log.transactionId && appliedIds.has(log.transactionId)) {
      skipped += 1;
      onSkip?.(log, ReplaySkipReasons.DUPLICATE);
      continue;
    }
    ...
    if (!handler) {
      ...
      skipped += 1;
      onSkip?.(log, ReplaySkipReasons.MISSING_HANDLER);
      continue;
    }
